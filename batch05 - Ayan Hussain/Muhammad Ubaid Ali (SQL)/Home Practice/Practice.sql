...
WHERE first_name like '_a%';
 ...
 DROP TABLE hr.candidatess;
IF NOT EXISTS (
    SELECT * FROM sys.schemas WHERE name = 'hr'
)
BEGIN
...
GO
 ...
 CREATE TABLE hr.candidatess(
 id int primary key identity,
 fullname varchar(100) not null);
 

 CREATE TABLE hr.employeess(
 id int primary key identity,
 fullname varchar(100) not null);


 INSERT INTO 
      hr.candidatess(fullname)
values
     ('Saad'),
     ('Mohsin'),
     ('Owais'),
...
insert into  
    hr.employeess(fullname)
values
     ('Saad'),
     ('Haseeb');


SELECT e.id as Emp_id,c.id as Can_id, c.fullname FROM hr.candidatess c
INNER JOIN hr.employeess e
